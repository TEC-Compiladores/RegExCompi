...
/**
 * Created by geraldtec on 05/03/16.
 */
public class GameWCreator extends AppCompatActivity implements Constants{
    private Intent _Screen;
    private Intent _IN;
    private TextView _ScreenTextView;
    public TextView _gameText;
    private TextView _creatorText;
    ...
    protected void onCreate(Bundle savedInstanceState){
        ...
        setContentView(R.layout.gamewcreator);
        _IN =getIntent();
    ...
    public void onClick(View v){
        switch (v.getId()){
            ...
            case R.id.bDeleteS:
                ...
                break;
            case R.id.bGotIt:
                _Screen=new Intent(this, GameWCreatorAfter.class);
                boolean flag=Validation.validar(getScreenPantallaCreator().toString());
                if (flag){
                    _Screen.putExtra("pattern", getScreenPantallaCreator().toString());
                    _Screen.putExtra("name",this.name);
                    _Screen.putExtra("gamename",this.gamename);
                    _Screen.putExtra("alfabeto",alfabeto);
                    startActivity(_Screen);
                    finish();
                }else{
                    Toast.makeText(getApplicationContext(), "Error, put a correct ReGex pattern!!", Toast.LENGTH_SHORT).show();
                }
    ...
    public String get_gameText() {
        return this._gameText.getText().toString();
    }

    public void set_gameText(String pgameText) {
        _gameText.setText(pgameText);
    }

    public String get_creatorText() {
        return this._creatorText.getText().toString();
    }

    public void set_creatorText(String pcreatorText) {
        this._creatorText.setText(pcreatorText);
    }
}
